 ...
 * Ноды, которые извлекаются из очереди, указатели на них кладутся в очередь
 * deleted_. Старшая половина этой очереди очищается каждый раз, когда очередь
 * превышает 10 записей.
 */
...
class MSQueue {
 ...
 private:
  ...
  // Clears half of nodes that were marked as deleted, if their count more than
  // 10 Thus it deletes 5 oldest marked deleted.
  void CollectGarbage() {
