...
class QueueSpinLock {
 ...
 private:
  void Acquire(Guard* waiter) {
    ...
    Ll("Start Acquire for waiter: %p (%s)\n", (void*)waiter, s);
    delete s;
    ...
    do { // CAS-loop
      ...
      if (success) {
        if (prev_tail != nullptr) {
          waiter->locked = true;
          Ll("Set waiter->locked=true: %p\n", (void*)waiter);
          prev_tail->next = waiter;
          Ll("Set prev_tail->next: %p for prev_tail: %p\n", (void*)waiter, (void*)prev_tail);
        } else {
          waiter->locked = false;
          Ll("First node set: %p\n", (void*)waiter);
        }
        Ll("Successfully changed tail from %p to %p\n", (void*)prev_tail, (void*)waiter);
      } else {
        Ll("Cannot change tail from %p to %p\n", (void*)prev_tail, (void*)waiter);
      }
    } while (!success);

    char* ss = PrintChain(waiter);
    Ll("Before spin for waiter: %p (%s)\n", (void*)waiter, s);
    delete(ss);

    twist::ed::SpinWait spin_wait;
    while (waiter->locked) {
      spin_wait();
      Ll("Spinning waiter: %p\n", (void*)waiter);
    }
    Ll("After spin for waiter: %p\n", (void*)waiter);
  }

  void Release(Guard* owner) {
    char* s = PrintChain(owner);
    Ll("Release: Start for owner: %p (%s)\n", (void*)owner, s);
    delete s;
